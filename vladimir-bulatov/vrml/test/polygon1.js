...
function shift_vertices2(v, comp){
  var n = v.length/comp;
  for(var c = 0; c < comp; c++){
    var off = c*n;
    var v0 = new SFVec3f(v[off].x,v[off].y,v[off].z);    
    for(var i = 0; i < n-1; i++){
      v[i+off] = v[(i+1) + c*off];
    }
    v[n-1+off] = v0;
...
function makeIFS6(){
  ...
  if(intersection3D(vertex[0],vertex[5],vertex[3],vertex[4],res6)){
    vertex[6] = res6;
